"""Automatic Lovelace resource registration for abstp_controller."""
...
RESOURCE_BASE_URL = f"/{DOMAIN}/abstp-player-card.js"
RESOURCE_TYPE = "module"
HASH_PREFIX_LENGTH = 12


class LovelaceResourcesCollection(Protocol):
    """Protocol for Lovelace resources collection in storage mode."""
...
class LovelaceData(Protocol):
    """Protocol for Lovelace component registry data."""

    resources: LovelaceResourcesCollection | None


def compute_frontend_hash(www_dir: Path | str) -> str | None:
    """Derive a cache-busting token from the frontend bundle content."""
    js_path = Path(www_dir) / "abstp-player-card.js"
...
def build_resource_url(content_hash: str | None) -> str:
    """Build the Lovelace resource URL with a cache-busting version query."""
    if content_hash:
...
def _url_matches_base(url: str) -> bool:
    """Check whether a Lovelace resource URL matches the abstp card base path."""
...
async def _ensure_loaded(resources: LovelaceResourcesCollection) -> None:
    """Ensure resource collection storage is loaded before reading or writing."""
...
async def async_register_resource(
    ...
    """Ensure the Lovelace resource registry references the latest frontend bundle."""
    ...
    for item in items:
        ...
        if url == target_url:
            LOGGER.debug("Abstp Lovelace resource already up to date")
            return
        ...
        if item_id:
            try:
                ...
                LOGGER.debug("Removed stale Abstp Lovelace resource")
            except (ValueError, TypeError, HomeAssistantError) as err:
                LOGGER.error("Failed to remove stale Abstp Lovelace resource: %s", err)
                return
        break
    ...
    try:
        ...
        LOGGER.info("Abstp Lovelace resource registered: %s", target_url)
    except (ValueError, TypeError, HomeAssistantError) as err:
        LOGGER.error("Failed to register Abstp Lovelace resource: %s", err)


async def async_unregister_resource(hass: HomeAssistant) -> None:
    """Unregister the abstp custom Lovelace card resource on component removal."""
    lovelace = cast("LovelaceData | None", hass.data.get("lovelace"))
    if lovelace is None:
        return
    ...
    for item in items:
        if _url_matches_base(item.get("url", "")):
            ...
            if item_id:
                try:
                    await resources.async_delete_item(item_id)
                    LOGGER.info("Abstp Lovelace resource removed successfully")
                except (ValueError, TypeError, HomeAssistantError) as err:
                    LOGGER.error(
                        "Failed to unregister Abstp Lovelace resource: %s", err
                    )
            break
